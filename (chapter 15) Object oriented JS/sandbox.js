...
class User {
    ...
    //определяем методы обьекта (методы определяются после ф-ции 'constructor" и только регулярными ф-циями, потому что при использованиии .this стрелочная ф-ция отссылается на глобальное окно)
    login(){
...
class Admin extends User{ //наследут совйства и методы от класса User
    constructor(username, email, title){
        super(username, email) //если у нас есть новые свойства, то обязательно прописывать новый constructor, при этом что бы новый класс взял свойства наследуемого нужен метод super()  внутри которого будут свойства которые мы хотим наследовать, то есть super вызывает конструктор родительского класса
        this.title = title
    }
...
//124 Prototipe inharitance (как наследовать без super(), тут метод super() недоступен, потому что мы не использовали class, а создали его функцией)
